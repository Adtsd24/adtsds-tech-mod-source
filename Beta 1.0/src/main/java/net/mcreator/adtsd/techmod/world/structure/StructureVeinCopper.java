...
package net.mcreator.adtsd.techmod.world.structure;
...
public class StructureVeinCopper extends ElementsADtsdsTechMod.ModElement {
	public StructureVeinCopper(ElementsADtsdsTechMod instance) {
		super(instance, 10);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((random.nextInt(1000000) + 1) <= 0) {
			...
			for (int a = 0; a < count; a++) {
				...
				require = Blocks.FLOWING_LAVA.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				require = Blocks.LAVA.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				if (!blockCriteria)
					continue;
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("adtsdstechmod", "veincopper"));
				if (template == null)
					return;
