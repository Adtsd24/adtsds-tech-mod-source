...
public class StructureGraniteChunkUnderground extends ElementsADtsdsTechMod.ModElement {
	public StructureGraniteChunkUnderground(ElementsADtsdsTechMod instance) {
		super(instance, 411);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 10000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("adtsdstechmod", "chunk_granite"));
				if (template == null)
					return;
				Rotation rotation = Rotation.values()[random.nextInt(3)];
				Mirror mirror = Mirror.values()[random.nextInt(2)];
				BlockPos spawnTo = new BlockPos(i, j + -15, k);
